...
(async function() {
    ...
    async function handleAutoConfig() {
        ...
        if (configBtn) {
            addToLog("✅ Botão 'Autoconfigurar' encontrado! Clicando...");
            await clickAndSleep(configBtn, 1000);
            lastActionTimestamp = Date.now();
        } else {
            addToLog("❌ Botão 'Autoconfigurar' não encontrado na tela.");
        }
    }
    ...
    function findButtonByPartialText(partialText, scope = document) { return Array.from(scope.querySelectorAll("button")).find(b => b.textContent.toLowerCase().includes(partialText.toLowerCase())); }
    ...
    function createHUD() {
        const allCSS = `
            ...
            .phbot-header { padding-bottom: 10px; border-bottom: 1px solid rgba(255, 255, 255, 0.1); cursor: move; }
            ...
            .phbot-input { background: rgba(0,0,0,0.3); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 6px; color: #fff; padding: 8px; font-family: 'Montserrat', sans-serif; font-size: 13px; box-sizing: border-box; }
        ...
        const daycareBtn = newElement('button', { className: 'phbot-button' });
        daycareBtn.innerHTML = `${icons.daycare}<span>Upar na Creche</span>`;
        daycareBtn.onclick = () => executeDaycareLogic(true);
        const autoDaycareBtn = createActionButton("Ativar Auto-Creche", icons.daycare, settings.isAutoDaycareEnabled, () => { settings.isAutoDaycareEnabled = !settings.isAutoDaycareEnabled; return settings.isAutoDaycareEnabled; });
